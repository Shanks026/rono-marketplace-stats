...
import { Badge } from '@/components/ui/badge' // will add below
...
export default function TaskPreviewCard({ preview, onClose, onSaved }) {
  const { createTask } = useTasks()
  const { goals } = useGoals()
  const { portals } = usePortals()

  const [summary, setSummary] = useState(preview.summary || '')
  const [goalIds, setGoalIds] = useState(preview.goal_ids || [])
  const [confidence, setConfidence] = useState(preview.confidence || [])
  const [portalId, setPortalId] = useState(preview.portal_id || '')
  const [taskType, setTaskType] = useState(preview.task_type || 'other')
  const [status, setStatus] = useState(preview.status || 'todo')
  const [effortHours, setEffortHours] = useState('')
  const [jiraRef, setJiraRef] = useState(preview.jira_ref || '')
  ...
  return (
    <Dialog open onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-xl">
        <DialogHeader>
          <DialogTitle className="text-base font-semibold">Review & log task</DialogTitle>
        </DialogHeader>
